...
import Dibujo (Dibujo, foldDib, rotar, espejar, rot45, apilar, juntar, encimar)
import FloatingPic (FloatingPic, Output, grid)

-- Interpretación de un dibujo
-- formulas sacadas del enunciado
interp :: Output a -> Output (Dibujo a)
interp fun fig = fun (foldDib fFigura fRotar fEspejar fRot45 fApilar fJuntar fEncimar fig)
    where   fFigura = id
            fRotar = id
            fEspejar = id
            fRot45 = id
            fApilar x y d1 d2 = d1
            fJuntar x y d1 d2 = d1
            fEncimar d1 d2 = d1
